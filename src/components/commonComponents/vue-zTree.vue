<style>
    ...
    .iconClassNode, .iconClassRoot {
        width: 16px;
        height: 15px;
        display: inline-block;
    }
    .root_open+a .iconClassRoot{
        background: url(../../images/ztree/zTreeStandard.png) no-repeat -110px -16px;
    }
    .root_close+a .iconClassRoot{
        background: url(../../images/ztree/zTreeStandard.png) no-repeat -110px 0px;
    }
    .bottom_open+a .iconClassRoot{
        background: url(../../images/ztree/zTreeStandard.png) no-repeat -110px -16px;
    }
    .center_open+a .iconClassRoot{
        background: url(../../images/ztree/zTreeStandard.png) no-repeat -110px -16px;
    }
    .bottom_close+a .iconClassRoot{
        background: url(../../images/ztree/zTreeStandard.png) no-repeat -110px 0px;
    }
	...
	components:{
        ...
        ztreeItem:{
            ...
            `<li :class="liClassVal">
				...
				<a :class="aClassVal" @click='Func(model)' @contextmenu.prevent='cxtmenufunc(model,nodes,trees)'>
				    <span :class="{loadSyncNode:model.loadNode==1}" v-if='model.loadNode==1'></span>
				    <span :class='model.iconClass' v-show='model.iconClass' v-else></span>
					<span class="node_name">{{model.name}}</span>
				</a>
				<ul :class="ulClassVal" v-show='model.isFolder'>
					<ztree-item v-for="(item,i) in model.children" :key='i' :callback='callback' :expandfunc='expandfunc' :cxtmenufunc='cxtmenufunc' :model.sync="item" :num.sync='i' root='1' :nodes.sync='model.children.length' :trees.sync='model.children'></ztree-item>
				</ul>
			</li>`
		}
	},
